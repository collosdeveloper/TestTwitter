...
public class PreCacheService extends IntentService {
    ...
    public void onHandleIntent(Intent intent) {
        ...
        if (DEBUG) {
            Log.v("Test_pre_cache", "starting the service, current time: " + Calendar.getInstance().getTime().toString());
        }

        // if they want it only over wifi and they are on mobile data
        if (getSharedPreferences("com.daiv.android.twitter_world_preferences",
                Context.MODE_WORLD_READABLE + Context.MODE_WORLD_WRITEABLE)
                .getBoolean("pre_cache_wifi_only", false) &&
                Utils.getConnectionStatus(this)) {

            if (DEBUG) {
                Log.v("Test_pre_cache", "quit for connection");
            }
        ...
        if (cursor.moveToFirst()) {
            if (DEBUG) {
                Log.v("Test_pre_cache", "found database and moved to first picture. cursor size: " + cursor.getCount());
            }
            boolean cont = true;
            do {
                ...
                if (!mCache.contains(profilePic)) {
                    ...
                    try {
                        ...
                        if (DEBUG) {
                            Log.v("Test_pre_cache", "found an exception while downloading profile pic");
                            e.printStackTrace();
                        }
                ...
                if (!imageUrl.equals("")) {
                    if (!mCache.contains(imageUrl)) {
                        try {
                            ...
                            if (DEBUG) {
                                Log.v("Test_pre_cache", "found an exception while downloading image");
                                e.printStackTrace();
                            }
            ...
            if (DEBUG) {
                Log.v("Test_pre_cache", "done with service. time: " + Calendar.getInstance().getTime().toString());
            }
        }
    }
