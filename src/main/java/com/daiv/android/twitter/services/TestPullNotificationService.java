...
import com.daiv.android.twitter.data.sq_lite.FavoriteUsersDataSource;
import com.daiv.android.twitter.data.sq_lite.HomeContentProvider;
...
import com.daiv.android.twitter.utils.NotificationUtils;
...
public class TestPullNotificationService extends Service {
    ...
    public void onCreate() {
        ...
        mBuilder =
                new NotificationCompat.Builder(this)
                        ...
                        .setContentTitle(getResources().getString(R.string.Test_pull) + (multAcc ? " - @" + settings.myScreenName : ""))
                        .setContentText(text)
                        .setOngoing(true)
    ...
    public BroadcastReceiver startPush = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            new Thread(new Runnable() {
                @Override
                public void run() {
                    while (TestPullNotificationService.shuttingDown) {
                        try {
                            Thread.sleep(1500);
                        } catch (Exception e) {
    ...
    public UserStreamListener userStream = new UserStreamListener() {
        ...
        public void onStatus(final Status status) {
            ...
            if(names.contains(settings.myScreenName)) {
                ...
                if (!status.isRetweet()) { // it is a normal mention
                    ...
                    mContext.sendBroadcast(new Intent("com.daiv.android.twitter.NEW_MENTION"));
            ...
            if (settings.liveStreaming && idsLoaded) {
                ...
                if (ids.contains(mId)) {
                    ...
                    pullUnread++;
                    sharedPreferences.edit().putInt("pull_unread", pullUnread).commit();
                    mContext.sendBroadcast(new Intent("com.daiv.android.twitter.NEW_TWEET"));
                    mContext.sendBroadcast(new Intent("com.daiv.android.twitter.UPDATE_NOTIF"));
                    mContext.sendBroadcast(new Intent("com.daiv.android.Test.UPDATE_WIDGET"));
        ...
        public void onDirectMessage(DirectMessage directMessage) {
            ...
            mContext.sendBroadcast(new Intent("com.daiv.android.twitter.NEW_DIRECT_MESSAGE"));
        }
